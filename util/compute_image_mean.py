...
def resizeDataset(filename):
    path = jpath(dataroot, filename)
    if isfile(path):
        imlist =  np.loadtxt(path,
...
path = jpath(dataroot, 'dataset_train.txt')
if isfile(path):
    imlist =  np.loadtxt(path,
                        dtype=str, delimiter=' ', skiprows=3, usecols=(0))
...
if args.save_resized_imgs:
    resizeDataset('dataset_test.txt')
    resizeDataset('dataset_val.txt')
